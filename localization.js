...
(function () {
  ...
  function set(selector, value) { const node=document.querySelector(selector); if(node && node.innerHTML!==value) node.innerHTML=value; }
  function applyLaunch() {
    const launch=document.querySelector('.start'); if(!launch) return;
    const title=launch.querySelector('h1'); const fixedTitle='<span class="title-orange">안전해</span><span class="title-white">수욕장</span>'; if(title && title.innerHTML!==fixedTitle) title.innerHTML=fixedTitle;
    const p=copy[state.lang];
    if(!p) return;
    const description=launch.querySelector('p'); if(description) description.innerHTML=p.startText;
    const labels=launch.querySelectorAll('.choose-label'); if(labels[0]) labels[0].textContent=p.chooseLanguage; if(labels[1]) labels[1].textContent=p.chooseTheme;
    const choices=launch.querySelectorAll('.choice'); if(choices[4]) choices[4].textContent=p.light; if(choices[5]) choices[5].textContent=p.dark;
    const go=launch.querySelector('.go'); if(go) go.textContent=p.begin;
  }
  function apply() {
    applyLaunch();
    ...
    const tips=document.querySelector('.tips'); if(tips && tips.dataset.locale!==state.lang) { tips.innerHTML=`<div class="tip">🦺<strong>${p.guide1}</strong><p>${p.guide1t}</p></div><div class="tip">🆘<strong>${p.guide2}</strong><p>${p.guide2t}</p></div>`; tips.dataset.locale=state.lang; }
